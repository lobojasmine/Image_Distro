...
def compute_per_candidate_timeline(venue_data_set):
    ...
    #print("listof per", per_candidate_timeline)
    return per_candidate_timeline
...
def parse_image_name(image_file_name):
    ...
    final_cpt=datetime.datetime.strptime(final_image, '%Y-%m-%d %H-%M-%S')

    # final_image_name=datetime.strptime(final_image, '%Y-%m-%d %H-%M-%S')
    # print("image_nm",final_image)
    #print("img_filenam",final_cpt)

    capture_ts = final_cpt
    return capture_ts
...
def process_venue(venue, venue_data_set):
    ...
    #print("percandtime", per_candidate_timeline.items())
    for candidate, timeline in per_candidate_timeline.items():
        ...
        for timeline_entry in timeline:
            ...
            #print("ip_imag_list1", ip_image_list)
            for image_entry in ip_image_list:
                #print("img_entry", image_entry[0])
                #print("timelin_ent",timeline[1])
                #print("timelin_ent2", timeline[2])
                start_ts = datetime.datetime.strptime(timeline[1], '%d/%b/%Y %H:%M:%S %p')
                end_ts = datetime.datetime.strptime(timeline[2], '%d/%b/%Y %H:%M:%S %p')
                #print("start",start_ts)
                #print("end",end_ts)
                if image_entry[0] >= start_ts and \
                        image_entry[0] <= end_ts:
                    #print("img_entry", image_entry)
                    str_venue=str(venue)
                    str_candidate=str(candidate)
                    image_path = os.path.join(
                        UNSORTED_BASE_FOLDER,
                        timeline[0], image_entry[1])
                    #print("img_path",image_path)
                    target_path = os.path.join(
                        SORTED_BASE_FOLDER, str_venue, str_candidate, image_entry[1])
                    #print("trg_path",target_path)
